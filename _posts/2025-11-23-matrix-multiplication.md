---
layout: post
title:  "Intuition Behind Matrix Multiplication"
date:   2025-11-23 10:00:00 +0800
categories:
- math
tags: [matrix, linalg]
---

# Intuition Behind Matrix Multiplication

```
It is my experience that proofs involving marices can be
shortened by 50% if one throws the matrices out.

--- Emil Artin
```

#### Highlights

* Matrix multiplication scales/rotates/skews a geometric plane (By remaining parallel and evenly spaced so that the origin remains fixed.)


---

### References

  * [A Programmer’s Intuition for Matrix Multiplication][]
  * [A Geometrical Understanding of Matrices][]
  * [Theory: Matrix Multiplication][]


[A Geometrical Understanding of Matrices]: https://gregorygundersen.com/blog/2018/10/24/matrices/
[A Programmer’s Intuition for Matrix Multiplication]: https://betterexplained.com/articles/matrix-multiplication/
[Theory: Matrix Multiplication]: https://textbooks.math.gatech.edu/ila/matrix-multiplication.html

